...
const MultipleSelector = ({ children, fields, handleSelected }) => {
  ...
  useEffect(() => {
    divRefs.current=Array.from({ length: fields.length }, () => createRef())
    setScheduleClicked(Array.from({ length: fields.length }, () => false))
  ...
  return (
    <div tabIndex={-1} ref={selectorBtnEl} onBlur={btnblur} className="relative w-full">
      ...
      >
        ...
        ></div>
        <div
          className="bg-transparent w-[40px] h-[40px] flex items-center justify-center absolute right-0 top-0 cursor-pointer rounded-tl-xl  rounded-bl-xl"
          onClick={() => {
            setIsOpen(!isOpen);
          }}
          >
        ...
        <div className="me-[40px] text-white text-sm width-full flex justify-start items-center relative z-50">
          <div
            className="h-[40px] whitespace-pre overflow-hidden flex items-center px-2 pl-[10px] cursor-pointer w-full"
            onClick={() => {
              setIsOpen(!isOpen);
            }}
          >
        ...
        {fields &&
          divRefs.current.map((ref, index) => (
            ...
            >
              ...
              <p className="pl-[26px] text-sm text-white">{fields[index]}</p>
            </div>
          ))}
      </div>
    </div>
