...
const CodeExecutorItemsUi = ({ searchParams }) => {
  ...
  const getvideoClipListsList = async (page) => {
    ...
    const config = {
      method: "GET",
      url: "/api/code-executor-activity",
      headers: {
  ...
  const apiDeleteByIdHook = useDeleteByIds("/api/code-executor-activity");
          ...
          <div className="w-full flex justify-end -m-1">
            ...
            `}
                ...
                onClick={() => createDefaultHook.create(
                  (data) => {
                    // success callback
                    router.push("/dashboard/code-executor-activity/" + data._id);
                  }
                )}
