...
const Page = ({ params }) => {
  ...
  const getpythonExecutorIssueListsList = async (page) => {
    ...
    const config = {
      ...
      params: {
        ...
        select: 'createdAt feedback user',
      },
    };
    setListLoading(true);
    try {
  ...
  const createSamplePythonExecutorIssueList = async () => {
    ...
    try {
      ...
      router.push(`/dashboard/coding-activity/${params.codingActivityId}/chat-feedback/${response.data._id}`);
    } catch (error) {
      if (error?.response?.status == 401) {
        toast.error(error.response.data.message + '. Login to try again.', {
          position: 'top-center',
  ...
  const apiDeleteByIdHook = useDeleteByIds("/api/chat-feedback");
              ...
              <div className="w-full flex justify-end pb-3 -m-1">
                <div className="p-1">
                  ...
                  >
                    ...
                    {createLoading ? 'Creating...' : 'New feedback'}
                  </button>
                </div>
                          ...
                          >
                            ...
                            <td className="px-6 py-4 text-right">
                              <div className="inline-flex space-x-1 items-center text-base font-semibold text-white">
                                {/* <Link href={`/dashboard/coding-activity/${params.codingActivityId}/pythonExecutorIssueList/${item._id}`}>
                            <button
                              type="button"
                              className="px-3 py-2 text-xs font-medium text-center inline-flex items-center text-white bg-yellow-700 rounded-lg hover:bg-yellow-800 focus:ring-4 focus:outline-none focus:ring-yellow-300 bg-yellow-600 hover:bg-yellow-700 focus:ring-yellow-800"
                            >
                              <svg
                                className="w-3 h-3 mr-1 text-white"
                                aria-hidden="true"
